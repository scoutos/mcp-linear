...
Deno.test("MCP Server - PUT /mcp/tools/linear-issue/{id} updates issue", async () => {
  ...
  const response = await mockRequest(handler, "/mcp/tools/linear-issue/TEST-123", "PUT", {
    title: "Updated Title",
...
Deno.test("MCP Server - POST /mcp/tools/linear-issue/{id}/comment adds comment", async () => {
  ...
  const response = await mockRequest(handler, "/mcp/tools/linear-issue/TEST-123/comment", "POST", {
